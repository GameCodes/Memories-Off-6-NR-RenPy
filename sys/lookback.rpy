...
screen text_history:
    on "show" action [SetField(persistent,"speaking",""),Stop("voice")]
    #use navigation
    tag menu
    modal True
    ...
    window:
        ...
        background "img/bgTextHistory-568h@2x.jpg"
        frame:
            background None
            maximum (760,480)
            align (.5,.5)
        ...
        key "rollforward" action If(adj.value==1.0,true=[Stop("voice"),Hide("text_history",transition=dissolve)],false=SetField(adj,"value",adj.value+.1))